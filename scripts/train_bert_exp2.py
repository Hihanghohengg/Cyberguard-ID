...
METADATA_PATH = PROJECT_ROOT / "models" / "experiment_2_focal" / "model_metadata.json"
...
class FocalLoss(nn.Module):
    ...
    def forward(self, inputs, targets):
        # 1. Unweighted per-sample cross entropy
        ce_loss = F.cross_entropy(inputs, targets, reduction='none')
        
        # 2. Probability of correct class
        pt = torch.exp(-ce_loss)
        
        # 3. Apply alpha class weights if provided
        if self.alpha is not None:
            # Pastikan tensor class weights berada pada device dan dtype yang sama
            alpha_device = self.alpha.to(inputs.device, dtype=inputs.dtype)
            alpha_t = alpha_device[targets]
            focal_loss = alpha_t * ((1 - pt) ** self.gamma) * ce_loss
            
            if self.reduction == 'mean':
                return focal_loss.sum() / alpha_t.sum().clamp_min(1e-8)
...
class FocalLossTrainer(Trainer):
    """Custom Trainer menggunakan Focal Loss untuk class imbalance."""
    ...
    def compute_loss(self, model, inputs, return_outputs=False, **kwargs):
        ...
        # Implementasi Focal Loss dengan gamma=2.0
        if self.class_weights is not None:
            loss_fct = FocalLoss(alpha=self.class_weights.to(model.device), gamma=2.0)
        else:
...
def main() -> None:
    ...
    train_aug_path = PROCESSED_DIR / "train.csv" # Murni tanpa augmentasi
    ...
    # Gunakan class weights identik dari baseline
    class_weights_array = np.array([0.44943928718566895, 1.506209135055542, 0.7785472869873047, 1.545347809791565, 5.569788455963135])
